...
if(!class_exists('WOR_Download')):
	require_once( 'helper/class-wor-download-helper.php');
	class WOR_Download extends WOR_Download_Helper{
      ...
      $this->load_html();
		...
		public function req_int(){
			wp_enqueue_style('wor-jquery-ui-style', 'https://ajax.googleapis.com/ajax/libs/jqueryui/1.8.2/themes/smoothness/jquery-ui.css');
			//wp_enqueue_style('wor-datatable-style', 'https://cdn.datatables.net/1.10.24/css/jquery.dataTables.min.css');    
      wp_register_style('tvc-dataTables-css', WOR_PLUGIN_URL.'/admin/css/dataTables.bootstrap4.min.css');
      wp_enqueue_style('tvc-dataTables-css');   
      //wp_enqueue_script('wor-datatable-js', 'https://cdn.datatables.net/1.10.24/js/jquery.dataTables.min.js');
      wp_enqueue_script( 'tvc-ee-dataTables-js', WOR_PLUGIN_URL . '/admin/js/jquery.dataTables.min.js', array( 'jquery' ), $this->version, false );
      wp_enqueue_script( 'tvc-ee-dataTables-1-js', WOR_PLUGIN_URL . '/admin/js/dataTables.bootstrap4.min.js', array( 'jquery' ), $this->version, false );
      wp_enqueue_script('jquery-ui-datepicker');			
		}

		public function load_html(){
			do_action('wor_header');
			$this->current_html();
			do_action('wor_footer');
		}
      ...
      $today_date = date( 'M-d-Y', strtotime( 'now' ));
				...
				<div class="wor-layout" id="wor-order-report">
					<div class="wor-section">
						<div class="wor-section-header">
							<h2 class="wor-section-header__title wor-section-header__header-item icon icon-download">Download woocommerce order reports</h2>
							...
							<div class="worwoocommerce order-date-select">
								...
								<form class="wor-date-range">
									<span><input type="text" name="start_date" value="<?php echo $start_date; ?>" placeholder="Start Date" class="example-datepicker start_date" /></span> - 
									<span><input type="text" name="end_date" value="<?php echo $today_date; ?>" placeholder="End Date" class="example-datepicker end_date" /></span>
									<span class="wor-filters-date__button"><button type="button" id="wor-order-report-btn" class="button button-primary button-large wor-order-re-genrate-button">Show Data</button></span>
	                <span class="wor-filters-date__button"><button type="button" id="wor-order-report-excel" class="button button-large wor-order-re-genrate-button">Download Data</button></span>
								</form>	
    	...
    	(function($){
          ...
          var data = {
            action: 'wor_show_data',
            start_date: start_date,
            end_date: end_date,
            wor_nonce: '<?php echo wp_create_nonce( 'wor_show_data_nonce' ); ?>'
          }; 
          if(start_date <=0){
        ...
        jQuery("#wor-order-report").on('click', '#wor-order-report-excel', function (e) {
          ...
          var data = {
            action: 'wor_download_data',
            start_date: start_date,
            end_date: end_date,
            wor_nonce: '<?php echo wp_create_nonce( 'wor_download_data_nonce' ); ?>',          
          }; 
          if(start_date <=0){
            $('#wor-date-range-msg').html('<div class="error"><p>Start Date is required.</p></div>');
            return false;
